...
def _recorded_url(m: aioresponses, index: int = -1) -> str:
    """Return the URL string of the *index*-th recorded request across all entries.

    Aioresponses stores requests keyed by (METHOD, URL). Multiple calls to the
    same URL accumulate under that key. This helper flattens the record list
    and returns the URL of the *index*-th call (0-based; -1 = last).
    """
    all_urls: list[str] = []
...
def _recorded_body(m: aioresponses, method: str, url: str, index: int = -1) -> dict:
    """Return the JSON body of the *index*-th recorded request to (method, url)."""
    calls = m.requests[(method.upper(), _URL(url))]
    return calls[index].kwargs.get("json") or {}


def _u(url: str) -> re.Pattern[str]:
    """Return a regex that matches *url* with an optional query string appended."""
    return re.compile(re.escape(url) + r"(\?.*)?$")


def _last_body(m: aioresponses) -> dict:
    """Return the JSON body of the most recent recorded request."""
    for (_method, _url), calls in reversed(list(m.requests.items())):
        if calls and calls[-1].kwargs.get("json") is not None:
...
class TestGetSystemDetails:
    ...
    async def test_returns_system_details_instance(self) -> None:
        with aioresponses() as m:
            ...
            details = await _make_system().get_details()
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get system details"):
                await _make_system().get_details()
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get live overview"):
                await _make_system().get_live_overview()
...
class TestGetDisplayedEvChargingModes:
    ...
    async def test_returns_enabled_charging_modes(self) -> None:
        with aioresponses() as m:
            ...
            modes = await _make_system().get_displayed_ev_charging_modes()
...
class TestGetEvChargers:
    ...
    async def test_returns_list_of_ev_charger_instances(self) -> None:
        with aioresponses() as m:
            ...
            chargers = await _make_system().get_ev_chargers()
    ...
    async def test_returns_empty_list_when_no_ev_chargers(self) -> None:
        with aioresponses() as m:
            ...
            assert await _make_system().get_ev_chargers() == []
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get EV chargers"):
                await _make_system().get_ev_chargers()
...
class TestGetEmsSettings:
    ...
    async def test_returns_ems_settings_in_auto_mode(self) -> None:
        with aioresponses() as m:
            ...
            settings = await _make_system().get_ems_settings()
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get EMS settings"):
                await _make_system().get_ems_settings()
...
class TestSetEmsMode:
    ...
    async def test_sets_auto_mode(self) -> None:
        with aioresponses() as m:
            ...
            await _make_system().set_ems_mode(auto=True)  # must not raise

            payload = _last_body(m)
    ...
    async def test_sets_manual_override(self) -> None:
        with aioresponses() as m:
            ...
            await _make_system().set_ems_mode(auto=False)
    ...
    async def test_raises_on_unexpected_status(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to set EMS mode"):
                await _make_system().set_ems_mode(auto=True)
...
class TestGetPrices:
    ...
    async def test_returns_market_prices_instance(self) -> None:
        with aioresponses() as m:
            ...
            result = await _make_system().get_prices(
    ...
    async def test_snaps_unaligned_end_up_to_next_1h_boundary(self) -> None:
        with aioresponses() as m:
            ...
            qs = _recorded_url(m, 0)
            assert "from=2024-06-01T00:00:00Z" in qs
            assert "to=2024-06-02T00:00:00Z" in qs
    ...
    async def test_snaps_15m_resolution_to_quarter_hour(self) -> None:
        with aioresponses() as m:
            ...
            qs = _recorded_url(m, 0)
            assert "from=2024-06-01T00:00:00Z" in qs
            assert "to=2024-06-01T00:30:00Z" in qs
            assert "resolution=15m" in qs
...
class TestGetEnergyToday:
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            m.get(_u(f"{_SYSTEM_BASE_V2}/energy-today"), payload={}, status=500)
...
class TestGetStatusAndAssets:
    ...
    async def test_returns_site_status_instance(self) -> None:
        with aioresponses() as m:
            ...
            result = await _make_system().get_status_and_assets()
...
class TestGetDeviceGateways:
    ...
    async def test_returns_gateway_instances(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_device_gateways_data(), status=200)
            result = await _make_system().get_device_gateways()
            assert len(result) == 1
    ...
    async def test_extracts_all_fields(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_device_gateways_data(), status=200)
            gw = (await _make_system().get_device_gateways())[0]
    ...
    async def test_sends_system_id_query_param(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_device_gateways_data(), status=200)
            await _make_system().get_device_gateways()
            assert _recorded_url(m, 0) == f"{self._URL}?systemId={FAKE_SYSTEM_ID}"

    async def test_empty_data(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload={"data": []}, status=200)
            assert await _make_system().get_device_gateways() == []

    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload={"error": "boom"}, status=500)
...
class TestGetActiveFeatures:
    ...
    async def test_returns_list_of_feature_codes(self) -> None:
        with aioresponses() as m:
    ...
    async def test_returns_empty_list_when_no_features(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._url()), payload={"features": []}, status=200)
            assert await _make_system().get_active_features(self._CUSTOMER_ID) == []

    async def test_handles_missing_features_key(self) -> None:
        with aioresponses() as m:
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._url()), payload={"error": "error"}, status=500)
            with pytest.raises(RequestError, match="Failed to get active features"):
                await _make_system().get_active_features(self._CUSTOMER_ID)
...
class TestGetEnergySavings:
    ...
    async def test_returns_savings_instance(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_energy_savings_data(), status=200)
            result = await _make_system().get_energy_savings()
            assert isinstance(result, HeartbeatSavings)
            assert result.savings_eur == pytest.approx(39.39)

    async def test_omits_query_params_when_dates_missing(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_energy_savings_data(), status=200)
    ...
    async def test_missing_field_yields_none(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload={}, status=200)
...
class TestGetWeather:
    ...
    async def test_returns_weather_data_instance(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_weather_data(), status=200)
            result = await _make_system().get_weather()
    ...
    async def test_missing_symbol_id_yields_placeholder_description(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_weather_data(), status=200)
            result = await _make_system().get_weather()
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get weather"):
                await _make_system().get_weather()
...
class TestGetPriceCustomizations:
    ...
    async def test_returns_all_prices(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_price_customizations_data(), status=200)
            result = await _make_system().get_price_customizations()
...
class TestGetComparisonPrice:
    ...
    async def test_returns_price_and_siteid_query(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_comparison_price_data(), status=200)
            result = await _make_system().get_comparison_price()
            assert isinstance(result, ComparisonPrice)
            assert result.price_eur_per_kwh == pytest.approx(0.274)
            assert f"siteId={FAKE_SYSTEM_ID}" in _recorded_url(m, 0)
...
class TestGetPriceGuarantee:
    ...
    async def test_returns_guarantee_and_systemid_query(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_price_guarantee_data(), status=200)
            ...
            assert f"systemId={FAKE_SYSTEM_ID}" in _recorded_url(m, 0)
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get price guarantee"):
                await _make_system().get_price_guarantee(self._CUSTOMER_ID)
...
class TestGetWallboxes:
    ...
    async def test_returns_list_of_wallboxes(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_wallboxes_data(), status=200)
            boxes = await _make_system().get_wallboxes()
            assert len(boxes) == 1
    ...
    async def test_gridx_hardware_id_is_none_on_site_scoped_response(self) -> None:
        with aioresponses() as m:
    ...
    async def test_returns_empty_list_when_none(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=[], status=200)
            assert await _make_system().get_wallboxes() == []

    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload={}, status=500)
            with pytest.raises(RequestError, match="Failed to get wallboxes"):
                await _make_system().get_wallboxes()
...
class TestGetSmartMeter:
    ...
    async def test_returns_flattened_fields(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_smart_meter_data(), status=200)
            result = await _make_system().get_smart_meter()
    ...
    async def test_handles_missing_arrays(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload={"siteId": FAKE_SYSTEM_ID}, status=200)
            result = await _make_system().get_smart_meter()
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get smart meter"):
                await _make_system().get_smart_meter()
...
class TestGetSelfSufficiencyEvents:
    ...
    async def test_query_params(self) -> None:
        with aioresponses() as m:
            ...
            assert "from=2026-08-01T05:30:00.000Z" in url
            assert "to=2026-08-01T06:45:00.999Z" in url
...
class TestGetSiteDetails:
    ...
    async def test_returns_extended_fields(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_site_details_data(), status=200)
            result = await _make_system().get_site_details()
    ...
    async def test_returns_newly_mapped_fields(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_site_details_data(), status=200)
            result = await _make_system().get_site_details()
    ...
    async def test_handles_missing_optional_fields(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload={"id": FAKE_SYSTEM_ID}, status=200)
            result = await _make_system().get_site_details()
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get site details"):
                await _make_system().get_site_details()
...
class TestGetCustomer:
    ...
    async def test_returns_full_customer(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_customer_data(), status=200)
            result = await _make_system().get_customer(self._CUSTOMER_ID)
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get customer"):
                await _make_system().get_customer(self._CUSTOMER_ID)
...
class TestGetSubscriptions:
    ...
    async def test_returns_all_four_subscription_types(self) -> None:
        with aioresponses() as m:
    ...
    async def test_url_uses_identity_host(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_subscriptions_data(), status=200)
            await _make_system().get_subscriptions(self._CUSTOMER_ID)
            assert "customer-identity" in _recorded_url(m, 0)
            assert "heartbeat" not in _recorded_url(m, 0)
...
class TestGetNotifications:
    ...
    async def test_returns_notifications_and_query(self) -> None:
        with aioresponses() as m:
            m.get(_u(_USERS_ME_URL), payload=make_user_data(), status=200)
            m.get(_u(self._URL), payload=make_notifications_data(), status=200)
            result = await _make_system().get_notifications()
    ...
    async def test_empty_list_when_no_notifications(self) -> None:
        with aioresponses() as m:
            m.get(_u(_USERS_ME_URL), payload=make_user_data(), status=200)
            m.get(_u(self._URL), payload={"data": []}, status=200)
            assert (await _make_system().get_notifications()).notifications == []
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get notifications"):
                await _make_system().get_notifications()
...
class TestGetNotificationSettings:
    ...
    async def test_returns_settings_with_channel_toggles(self) -> None:
        with aioresponses() as m:
            m.get(_u(_USERS_ME_URL), payload=make_user_data(), status=200)
            m.get(_u(self._URL), payload=make_notification_settings_data(), status=200)
            result = await _make_system().get_notification_settings()
...
class TestGetImpactOverview:
    ...
    async def test_returns_instance(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_impact_overview_data(), status=200)
            result = await _make_system().get_impact_overview()
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get impact overview"):
                await _make_system().get_impact_overview()
...
class TestGetEnergyTrader:
    ...
    async def test_returns_instance_and_query_param(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_energy_trader_data(), status=200)
            result = await _make_system().get_energy_trader()
    ...
    async def test_raises_on_server_error(self) -> None:
        with aioresponses() as m:
            ...
            with pytest.raises(RequestError, match="Failed to get energy trader"):
                await _make_system().get_energy_trader()
...
class TestGetHeartbeatPrices:
    ...
    async def test_returns_all_five_windows(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_heartbeat_prices_data(), status=200)
            result = await _make_system().get_heartbeat_prices()
    ...
    async def test_correct_price_attribution_year_window(self) -> None:
        with aioresponses() as m:
    ...
    async def test_energy_and_cost_values_extracted(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_heartbeat_prices_data(), status=200)
            y = (await _make_system().get_heartbeat_prices()).year
    ...
    async def test_implausibility_flag_per_window(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_heartbeat_prices_data(), status=200)
    ...
    async def test_null_regional_fields_pass_through(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_heartbeat_prices_data(), status=200)
            y = (await _make_system().get_heartbeat_prices()).year
    ...
    async def test_url_and_site_id_query_param(self) -> None:
        with aioresponses() as m:
            m.get(_u(self._URL), payload=make_heartbeat_prices_data(), status=200)
            await _make_system().get_heartbeat_prices()
            assert f"siteId={FAKE_SYSTEM_ID}" in _recorded_url(m, 0)
...
class TestGetHeartbeatAiSummary:
    ...
    async def test_peak_price_avoided_absent_yields_none(self) -> None:
        with aioresponses() as m:
            body = make_heartbeat_ai_summary_data("1M")
            body["peakPriceAvoided"] = None
            m.get(_u(self._URL), payload=body, status=200)
            result = await _make_system().get_heartbeat_ai_summary()
...
class TestGetOptimizations:
    async def test_returns_optimization_events(self) -> None:
        with aioresponses() as m:
            ...
            result = await _make_system().get_optimizations(
    ...
    async def test_slot_count_and_end_time(self) -> None:
        with aioresponses() as m:
            m.get(_u(_OPTIMIZATIONS_URL), payload=make_optimizations_data(), status=200)
            result = await _make_system().get_optimizations(
                datetime.datetime(2026, 6, 1, 10),
    ...
    async def test_null_market_price_stays_none(self) -> None:
        with aioresponses() as m:
            m.get(_u(_OPTIMIZATIONS_URL), payload=make_optimizations_data(), status=200)
            result = await _make_system().get_optimizations(
                datetime.datetime(2026, 6, 1, 10),
    ...
    async def test_url_and_query_params(self) -> None:
        with aioresponses() as m:
            m.get(_u(_OPTIMIZATIONS_URL), payload=make_optimizations_data(), status=200)
            await _make_system().get_optimizations(
